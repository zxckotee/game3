...
// Директория с файлами адаптеров данных
...
if (!fs.existsSync(dataDirectory)) {
  console.log(`Директория ${dataDirectory} не существует`);
  return;
}

// Получаем список адаптеров
const adapterFiles = fs.readdirSync(dataDirectory)
  .filter(file => file.includes('-adapter.js') && !file.includes('.cjs'));
...
console.log(`Преобразовано файлов: ${adapterFiles.length}`);