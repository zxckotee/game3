...
1. **Самоподписанный сертификат** - не от доверенного CA
2. **Несоответствие домена** - сертификат для `culty.ru`, а заходите через `localhost`

**Решения:**
- **Быстрое:** В браузере нажмите "Дополнительно" → "Перейти на localhost (небезопасно)"
- **Для разработки:** Добавьте в hosts файл:
  ```
  127.0.0.1 culty.ru
  127.0.0.1 www.culty.ru
  ```
- **Для продакшена:** Получите сертификат от доверенного CA
