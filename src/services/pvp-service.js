...
class PvPService {
  ...
  static async getRoomState(roomId, userId = null, lastActionId = 0) {
    try {
      console.log(`[PvP Backend DEBUG] 🔄 getRoomState вызвана для комнаты ${roomId}, userId: ${userId}, lastActionId: ${lastActionId}`);
      ...
      if (userId) {
        ...
        if (participant) {
          console.log(`[PvP Backend DEBUG] 👤 Обновление характеристик игрока ${userId} при получении состояния комнаты ${roomId}`);
          await this.savePlayerStats(userId, roomId);
        }
      }
      
      // Получаем детали комнаты
      console.log(`[PvP Backend DEBUG] 🏠 Вызываем getRoomDetails для комнаты ${roomId}`);
      const roomDetails = await this.getRoomDetails(roomId);
      
      console.log(`[PvP Backend DEBUG] 📋 Результат getRoomDetails:`, JSON.stringify(roomDetails, null, 2));
      
      if (roomDetails.room) {
        console.log(`[PvP Backend DEBUG] 🏠 Статус комнаты: "${roomDetails.room.status}"`);
        console.log(`[PvP Backend DEBUG] 🏆 Команда-победитель: ${roomDetails.room.winner_team || 'не определена'}`);
      }
      
      if (roomDetails.rewards) {
        console.log(`[PvP Backend DEBUG] 🎁 Награды в ответе:`, roomDetails.rewards);
      } else {
        console.log(`[PvP Backend DEBUG] ❌ Награды отсутствуют в getRoomDetails`);
      }
